...
PETFORGE_STAR_DAILY_LIMIT=10
PETFORGE_GITHUB_REPO_URL=https://github.com/CHELSEADOPAMIN/CodexPetss
PETFORGE_PAID_PACK_GENERATIONS=5
PETFORGE_PAID_PACK_AMOUNT_CENTS=200
PETFORGE_PUBLIC_URL=
PETFORGE_JOBS_DIR=
STRIPE_SECRET_KEY=
```
...
By default, each client IP can generate up to 5 pets per day. If OpenAI vision verifies a GitHub Star screenshot for `PETFORGE_GITHUB_REPO_URL`, that IP gets `PETFORGE_STAR_DAILY_LIMIT` included generations per day. `STRIPE_SECRET_KEY` enables the paid pack flow; the default is $2 for 5 extra generations.
...
PETFORGE_PAID_PACK_GENERATIONS=5
PETFORGE_PAID_PACK_AMOUNT_CENTS=200
PETFORGE_PUBLIC_URL=https://your-domain.example
...
默认情况下，同一个客户端 IP 每天最多生成 5 次。如果 OpenAI 视觉模型验证了 `PETFORGE_GITHUB_REPO_URL` 的 GitHub Star 截图，这个 IP 每天会有 `PETFORGE_STAR_DAILY_LIMIT` 次免费额度。配置 `STRIPE_SECRET_KEY` 后会启用付费包，默认 2 美元买 5 次额外生成。
